...
**Confirmado:** tendencia lineal creciente en ambos sexos (p < 0.001). La pendiente en mujeres es **2.5x mayor** que en hombres. La pendiente femenina es +1.085pp/año y la masculina +0.442pp/año, calculadas con 9 puntos anuales. *Limitación:* el Cochran-Armitage no se acompaña de IC95 bootstrap en este informe — la inferencia es direccional, no de magnitud exacta.
...
| **Diferencia** | **+9.5pp** | |
| Chi² (conteos no ponderados) | ~580 | p < 0.001 |
| Chi² (conteos ponderados, referencia) | 918.84 | p < 0.001 |
| Odds ratio | 1.549 | |
...
Análisis estratificado por sexo × raza (8 categorías CDC) usando solo años donde `raceeth` está disponible (2007+, ya que la columna derivada no existe en 2005). Pre = 2007-2009, post = 2017-2021.
...
![Simpson heatmap](reports/figures/fig9_simpson_heatmap.png){#fig-9}
...
1. **El deterioro de salud mental adolescente en EE.UU. (2005-2021) es real y estadísticamente significativo.** La regresión logística confirma un OR de 1.93 entre 2005 y 2021.
...
4. **La dosis-respuesta screen time tiene forma de J, no monotónica.** OR ajustada para 1h es 0.95 (menor que no-uso); la aceleración ocurre a partir de 3h. Los 5+h tienen OR 2.14 (IC95 1.90-2.41) en 2019. Caveat: transversal, e IC95 no corrigen por el diseño complejo.
...
- **Diseño muestral complejo no modelado:** las regresiones usan `weight` como `freq_weights` pero ignoran `stratum`+`psu`. Los IC95 de las OR son más estrechos de lo correcto; un análisis riguroso requeriría `cov_type='cluster'` o el módulo `samplics`. Las conclusiones direccionales se mantienen.
- **Chi-cuadrado pre/post:** ahora se reportan dos versiones: con conteos no ponderados (~580) y con conteos ponderados (919) como referencia. Ambos dan p < 0.001.
